...
var showAdminProductsTemplate = __webpack_require__(44);
var showCartTemplate = __webpack_require__(45);
var showCheckoutTemplate = __webpack_require__(46);
...
var pushItemsToCart = function pushItemsToCart() {
  ...
  store.cart.forEach(function (item) {
    for (var i = 0; i < store.products.length; i++) {
      if (store.products[i].id === item.product_id && store.products[i].status !== 'hidden') {
        cartFix.push(item);
      }
    }
  });
...
var sellerAdmin = function sellerAdmin(data) {
  var sellerProducts = [];
  for (var i = 0; i < store.products.length; i++) {
  ...
  var showProductsHTML = showAdminProductsTemplate({ products: sellerProducts });
...
var createNewProductSuccess = function createNewProductSuccess(data) {
  $('#createProductModal').modal('hide');
  $('.text-field-product').val('');
  $('#UiSuccess').text('Create successful, now make more.').fadeIn('fast').delay(5000).fadeOut('slow');
};

var createNewProductFailure = function createNewProductFailure(data) {
  $('#UiFailure').text('Oops! Something went wrong!').fadeIn('fast').delay(3000).fadeOut('slow');
};
...
var updateProductFailure = function updateProductFailure(data) {
  $('#UiFailure').text('Oops! Something went wrong!').fadeIn('fast').delay(3000).fadeOut('slow');
};
...
var deleteProductFailure = function deleteProductFailure(data) {
  $('#UiFailure').text('Oops! Something went wrong!').fadeIn('fast').delay(3000).fadeOut('slow');
};
...
var orderEvents = __webpack_require__(47);
var productEvents = __webpack_require__(48);
var stripeEvents = __webpack_require__(8);
...
var onSignIn = function onSignIn(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  api.signIn(data).then(ui.signInSuccess).catch(ui.signInFailure).then(productsApi.showAllProducts).then(productsUi.showAllProductsSuccess).then(ordersApi.showAllOrders).then(ordersUi.showAllOrdersSuccess).then(productsUi.carriageBoy).then(productsUi.sellerAdmin).catch(productsUi.showAllProductsFailure);
};
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "\n<!-- this is the handlebars for products when the page loads. Products will\nbe displayed, through this mechanism, on the landingPage -->\n<tr data-id=\"Row-"
    ...
    + "</p></td>\n <td><p>"
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.rating : stack1), depth0))
    + " Glarbles</p></td>\n\n\n<!-- Add to cart form with a hidden input that contains the product id and an input\nfor quanity. When submitted, this form will trigger a function to add the\nitem with the specified quanity to the shopping cart -->\n\n<td>\n\n  <a class=\"load-edit-btn btn\" data-toggle=\"modal\" data-target=\".bs-edit-delete\" href=\"#\"\n    class=\"list-group-item list-group-item-action\" id=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\">Edit or Delete</a>\n\n</td>\n</tr>\n\n\n";
},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1;
...
var onSellerMenuButton = function onSellerMenuButton() {
  $('.landingPage').hide();
  $('#cartPage').hide();
  $('#checkoutPage').hide();
...
var onCreateProduct = function onCreateProduct(event) {
  ...
  api.createNewProduct(data).then(ui.createNewProductSuccess).then(api.showAllProducts).then(ui.showAllProductsSuccess).then(ui.sellerAdmin).catch(ui.createNewProductFailure);
};

var onEditProduct = function onEditProduct(event) {
  event.preventDefault();
  var id = document.getElementById('id-store').value;
  var url = document.getElementById('image-link').value;
  var name = document.getElementById('name-text').value;
  var description = document.getElementById('description-text').value;
  var category = document.getElementById('category-text').value;
  ...
  api.updateProduct(data, id).then(ui.updateProductSuccess).then(api.showAllProducts).then(ui.showAllProductsSuccess).then(ui.sellerAdmin).catch(ui.updateProductFailure);
};
...
var onDeleteProduct = function onDeleteProduct(event) {
  ...
  api.deleteProduct(data, id).then(ui.deleteProductSuccess).then(api.showAllProducts).then(ui.showAllProductsSuccess).then(ui.sellerAdmin).catch(ui.deleteProductFailure);
};
...
var addHandlers = function addHandlers() {
  ...
  $('#editDeleteModal').on('show.bs.modal', function (event) {
    var converter = $(event.relatedTarget);
    for (var i = 0; i < store.products.length; i++) {
